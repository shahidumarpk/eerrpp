...
class mod_projects extends CI_Model {
  ...
  public function project_messages($data){
		...
		$insrt_data = array(
			...
			'post_type'=>$this->db->escape_str(trim('1')),
	...
	public function get_project_messages($project_id){
		...
		//echo"<pre>"; print_r($get_project_messages_arr['project_messages_result']); exit;
	 ...
	 public  function daily_tasks(){
		...
		//group by day to show a single ammount per day
			...
			$query = "SELECT
						...
						date(inno_project_task.created_date) as formated_created_date ,
						GROUP_CONCAT(inno_project_task.title) as task_title
						
						FROM
											inno_project_task
											
											INNER JOIN inno_projects ON inno_projects.id =inno_project_task.project_id
											
											
						WHERE

						inno_projects.customer_id=".$customer_id." AND 
