	...
	public function add_income($data){
		...
		extract($data);
		   ...
		   $ins_data = array(
			...
			'income_usd' => $this->db->escape_str(trim($income_usd)),
		    'income' => $this->db->escape_str(trim($income_pkr)),
	...
	public function add_expense($data){
		...
		extract($data);
		   ...
		   $ins_data = array(
			...
			'expense_usd' => $this->db->escape_str(trim($expense_usd)),
		    'expense' => $this->db->escape_str(trim($expense_pkr)),
	  ...
	  if($this->input->post('from_date') !="" && $this->input->post('to_date')!="")
		{
			...
			$this->db->dbprefix('projects_account');
			$this->db->select('id,branch_id,income,description,dated,income_usd'); 
            $this->db->from('projects_account'); 
			$this->db->where('income !=',0.00);
		    $this->db->where("dated BETWEEN '".$from_date."' AND '".$to_date."'"); 
		    $this->db->order_by('dated', DESC);
...
//mzm
	function get_income_chart(){
		...
		//group by day to show a single ammount per day
			...
			$query = "SELECT
						inno_projects_account.id,
						Sum(inno_projects_account.income) AS income,
						Sum(inno_projects_account.income_usd) AS income_usd,
						inno_projects_account.expense_usd,
						inno_projects_account.expense,
						GROUP_CONCAT(inno_projects_account.description) as project_title,
						
						date(inno_projects_account.created_date) as formated_created_date
						FROM
						inno_projects_account
						WHERE
						inno_projects_account.expense = 0 AND
						inno_projects_account.income > 0 AND
						inno_projects_account.created_date >= ( CURDATE() - INTERVAL 10 DAY )
