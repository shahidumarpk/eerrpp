...
//	exit;
	...
	//Get Unread Notifications Admin Section
	public function get_admin_unread_data($admin_id){
		
		$this->db->dbprefix('admin');

		$this->db->select('admin.id');
		$this->db->join('messages','admin.id=messages.from');
	$this->db->where('messages.to',$admin_id);
		$get_admin = $this->db->get('admin');

		//echo $this->db->last_query();
		
		$row_admin['admin_unread_count'] = $get_admin->num_rows;
	...
	//Get Admin Bank Information
	public function get_admin_bank_data($admin_id){
		
		$this->db->dbprefix('admin');$this->db->select('admin.id,admin.bank_name,admin.branch_code,admin.account#,ibn#');

		$this->db->where('admin.id',$admin_id);
		$get_admin = $this->db->get('admin');

		//echo $this->db->last_query();
		$row_admin['admin_bank_arr'] = $get_admin->row_array();
		$row_admin['admin_bank_count'] = $get_admin->num_rows;
	...
	// Get admin COntact Inforamtion
	public function get_admin_contact_data($admin_id){
		
		$this->db->dbprefix('admin');$this->db->select('admin.id,admin.emergency_phone,admin.address,admin.phone');

		$this->db->where('admin.id',$admin_id);
		$get_admin = $this->db->get('admin');

		//echo $this->db->last_query();
		$row_admin['admin_contact_arr'] = $get_admin->row_array();
		$row_admin['admin_contact_count'] = $get_admin->num_rows;
	...
	public function add_new_user($data){
		...
		$ins_data = array(
		   ...
		   'father_name' => $this->db->escape_str(trim($father_name)),
		   'display_name' => $this->db->escape_str(trim($display_name)),
		   ...
		   'branch_code'=>$this->db->escape_str(trim($branch_code)),
		   'account#'=>$this->db->escape_str(trim($account_no)),
		    'ibn#'=>$this->db->escape_str(trim($ibn_no)),
		   ...
		   'designation' => $this->db->escape_str(trim($designation)),
		   'last_increament' => $this->db->escape_str(trim($last_increament)),
		   'ip' => $this->db->escape_str(trim($ip)),
		   'pc_no' => $this->db->escape_str(trim($pc_no)),
		   'desk_no' => $this->db->escape_str(trim($desk_no)),
	...
	public function task_detail_report(){
		...
		$row_detail_report['task_detail_report_arr'][$i]['total_time']= $row_task['total_time']; 
...
###############################################################################################
	...
	//print_letter
	public function print_letter($user_id,$type){
		
		
		
		
		$row_admin = $this->get_admin_profile($user_id);
		
		$user_arr = $row_admin['admin_profile_arr'];
		
		
 		$created_date = date('d, M Y');
	    $username = $user_arr['first_name']." ".$user_arr['last_name'];
		$father_name = $user_arr['father_name'];
	    $designation = $user_arr['designation'];
		$nic = $user_arr['nic'];
		$join_date = date('d, M Y', strtotime($user_arr['join_date']));
		$salary = number_format($user_arr['salary'],2,".",",");
		...
		if($type=="appointment"){
			...
			$get_email_data = $this->mod_email->get_email(11);
			$file_name= "Appointment_Letter.pdf";
			
			$templete_subject = $get_email_data['email_arr']['email_subject'];
			$templete_body = $get_email_data['email_arr']['email_body'];
			$search_arr = array('[TODAY_DATE]','[USERNAME]','[DESIGNATION]','[JOIN_DATE]','[SALARY]');
			$replace_arr = array($created_date,$username,$designation,$join_date,$salary);
			$templete_body = str_replace($search_arr,$replace_arr,$templete_body);
			
			//Update Status
			$upd_appointment_letter = array('appointment_letter' => $this->db->escape_str(trim(1)));
			$this->db->dbprefix('admin');
			$this->db->where('id',$user_id);
			$this->db->update('admin', $upd_appointment_letter);	
		
		}
		
		if($type=="bank"){
			
			$get_email_data = $this->mod_email->get_email(12);
			$file_name= "Bank_Opening_Letter.pdf";
			
			$templete_subject = $get_email_data['email_arr']['email_subject'];
			$templete_body = $get_email_data['email_arr']['email_body'];
			$search_arr = array('[TODAY_DATE]','[USERNAME]','[FATHER_NAME]','[NIC]','[SALARY]');
			$replace_arr = array($created_date,$username,$father_name,$nic,$salary);
			$templete_body = str_replace($search_arr,$replace_arr,$templete_body);
			
			
			//Update Status
			$upd_bank_oppening_letter= array('bank_oppening_letter' => $this->db->escape_str(trim(1)));
			$this->db->dbprefix('admin');
			$this->db->where('id',$user_id);
			$this->db->update('admin', $upd_bank_oppening_letter);
		}
		
		if($type=="access"){
			
			$get_email_data = $this->mod_email->get_email(13);
			$file_name= "User_Access_Letter.pdf";

			//Generate auto password;
			$auto_password = $this->mod_common->random_number_generator(6);

			//Update Status
			$upd_password= array('password' => $this->db->escape_str(trim(md5($auto_password))));
			$this->db->dbprefix('admin');
			$this->db->where('id',$user_id);
			$this->db->update('admin', $upd_password);
			...
			$templete_subject = $get_email_data['email_arr']['email_subject'];
			$templete_body = $get_email_data['email_arr']['email_body'];
			$search_arr = array('[Letter_Issue_Date]','[Emp_Name]','[URL]','[Username]','[Password]');
			$replace_arr = array($created_date,$username,$url,$user_name,$auto_password);
			$templete_body = str_replace($search_arr,$replace_arr,$templete_body);
			
			//Update Status
			$upd_user_access_letter= array('user_access_letter' => $this->db->escape_str(trim(1)));
			$this->db->dbprefix('admin');
			$this->db->where('id',$user_id);
			$this->db->update('admin', $upd_user_access_letter);
		}
		
		
		
		
		$pdfFilePath1 = "./letter_pdf/".$file_name;
		
		ini_set('memory_limit','32M'); // boost the memory limit if it's low <img class="emoji" draggable="false" alt="ðŸ˜‰" 
		...
		$pdf->WriteHTML($templete_body); // write the HTML into the PDF
	...
	public function change_status_active($user_id){
	...
	}//End change_status_active
	
	
	public function change_status_inactive($user_id){
