...
class mod_projects extends CI_Model {
   ...
   //Get All users
	...
	public function project_detail($project_id){
		...
		$this->db->dbprefix('projects');
		$this->db->select('projects.*,customers.first_name,customers.last_name,customers.id, project_attachments.attachment_name');
        $this->db->from('projects');
        $this->db->join('customers', 'projects.customer_id = customers.id');
        $this->db->join('project_attachments', 'project_attachments.project_id = projects.id');
		$this->db->where('projects.id',$project_id);
		$get_project_details = $this->db->get();
		$get_project_details_arr['project_details_result'] = $get_project_details->result_array();

		/*
		echo '<pre>';
		print_r($get_project_details_arr['project_details_result']);	
		exit;
		*/
			...
			$data = array();
			foreach($get_project_details_arr['project_details_result'] as $index=>$data){
				// 
				if(isset($data['attachment_name']))
					$attachments[]['attachment_name'] = $data['attachment_name'];
			}

		
		$get_project_details_arr['project_attachments'] = $attachments;
		// $get_project_details_arr['project_attachments'] = $attachments;
		// $get_project_details_arr['project_details_result']['attachment_name'];
		$get_project_details_arr['project_attachments_count'] = count($get_project_details_arr);
		
		 // $get_project_details_arr['project_details_result']['attachment_name']
		
		
		//$get_project_attachments->num_rows;
...
//		$get_project_details_arr['project_attachments_count'] = $get_project_attachments->num_rows;
		...
		$project_assign_id = explode(',', $get_project_details_arr['project_details_result'][0]['project_assign']);
	  ...
	  $message='To see complete message please click on the link below:<br /><a href="'.base_url().'projects/manage-projects/project-detail/'.$project_id.'">'.base_url().'projects/manage-projects/project-detail/'.$project_id.'</a>';
		...
		for($i=0; $i<count($_FILES['attachments']['name']); $i++)
		   ...
		   if($_FILES['attachments']['name'][$i] !=""){
			...
			$config['allowed_types'] = 'jpg|jpeg|gif|tif|tiff|png|doc|docx|xls|xlsx|pdf|txt||zip|rar|odt|csv|ppt|pptx';
			$config['max_size']	= '15000';
			$config['overwrite'] = true;
...
*/
	...
	public function project_assign($data){
		...
		$assign_team_arr= explode(',',$get_assign_team);
...
//exit;
   ...
   //show_all_tasks
	public function show_all_tasks(){
		...
		$this->db->dbprefix('project_task');
		$this->db->where('status',$this->input->post('search_status'));
		$this->db->order_by('id',DESC);
		$get_all_task= $this->db->get('project_task');
