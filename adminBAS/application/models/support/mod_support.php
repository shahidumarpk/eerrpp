...
class mod_support extends CI_Model {
	...
	public function ticket_reply($data){
		...
		$ins_data = array(
		   ...
		   'details' => $this->db->escape_str(addslashes(trim(nl2br($ticket_reply)))),
	...
	public function add_ticket($data){
	  ...
	  if($account_type=='guest'){
		...
		$ins_data = array(
		   'customer_id' => $this->db->escape_str(trim($new_customer_id)),
		   ...
		   'created_by' => $this->db->escape_str(trim($new_customer_id)),
	    ...
	    if($account_type=='guest'){
		      ...
		      $to_customer=$guest_email;
			
		}
		
		
	
	        $subject = $ticket_number.' '.stripslashes($subject) ;
