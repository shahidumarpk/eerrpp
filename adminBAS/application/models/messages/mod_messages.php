...
class mod_messages extends CI_Model {
	...
	public function payrolls_notification ($data)
	{
		/*echo '<pre>';
		print_r($data);exit;*/
		extract($data);
// Getting Type To save Data According to the Fine Type


/*
echo 'Printing ......... Amount <br>';
print_r($amount);
echo '<br>';
echo 'Printing ......... Date <br>';
print_r($transaction_date);
echo '<br>';
echo 'Printing ......... Type <br>';
print_r($type);
echo '<br>';
exit;
*/
...
$transaction_date = date("Y-m-d", strtotime($transaction_date));
				...
				if($type=='fine')
				{
				$notification_subject = "Fine Alert";
				$notification_message = "You have been fined Rs. $amount. Following are the details 
				Fine Amount : Rs. $amount
				<br/>
				Fine Date : $transaction_date
				<br/>
			...
			if($type=='tardy')
			{
				$notification_subject = "Manual Tardy Fine";
				$notification_message = "Manual tardy fine added for your account. Following are the details 
				Fine Amount : Rs. $amount
				<br/>
				Fine Date : $transaction_date
				<br/>
	...
	{
				$notification_subject = "Congrats Incentive Alert";
				$notification_message = "You have been awarded an incentive. Following are the details 
				Incentive Amount : Rs. $amount
				<br/>
...
// Checking Number of Selected Users for transaction Fine and Notification
$count=count($user_name);
	...
	$created_by = $this->session->userdata('admin_id');
			...
			for($i=0;$i<$count;$i++)
			{
			 $ins_notification[$i] = array(
					'to' => $this->db->escape_str(trim($user_name[$i])),
					'from' => $this->db->escape_str(trim($created_by)),
...
/*echo '<pre>';
print_r($ins_notification);
exit;*/
				   //Inserting the record into the database.
				  $this->db->dbprefix('messages');
				  $ins_into_db = $this->db->insert_batch('messages', $ins_notification);
	...
	public function get_inbox_message_detail($message_id){
		...
		for($i=0; $i<$row_message['message_detail_count']; $i++){
			...
			if($type=='c'){
				...
				if($row_message['message_detail_arr'][$i]['project_message_id'] !=""){
						
						
						$project_message_id= $row_message['message_detail_arr'][$i]['project_message_id'];
						
						$this->db->dbprefix('project_messages');
						$this->db->where('id',$project_message_id);
						$get_project_message= $this->db->get('project_messages');
						
						$row_get_project_message = $get_project_message->row_array();
						
						$row_message['message_detail_arr'][$i]['project_message']=$row_get_project_message['message'];
				...
				*/
				...
				elseif($col == 'subject'){
					 ...
					 if($aRow['notification']==1){
					  ...
					  }elseif($aRow['notification']==3){
						 
						 $notification='<span class="label btn-dark pull-right ">Milestone</span>';
