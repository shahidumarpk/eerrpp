                ...
                ?>
                  ...
                  <table class="table table-striped table-bordered table-hover" id="manage_cms_pages">
                    ...
                    <?php 
							...
							$total_allocatedtime+=$assign_task_arr[$i]['allocatedtime'];
							$total_consumedtime+=$assign_task_arr[$i]['consumedtime'];
					?>
                            <tr>
                             ...
                             <?php  if($assign_task_arr[$i]['status']==3 && $assign_task_arr[$i]['rating_status']==0 ){ ?><span class="label btn-red">?</span>  <?php  } ?>
								...
								if($assign_task_arr[$i]['allocatedtime'] !=0){ 
								 
								 if($assign_task_arr[$i]['allocatedtime'] >=60){ 
								 
								   $hours = floor($assign_task_arr[$i]['allocatedtime'] / 60);
								   $mints = ($assign_task_arr[$i]['allocatedtime'] % 60);
									...
									echo  "00:".$assign_task_arr[$i]['allocatedtime'];
								...
								if($assign_task_arr[$i]['task_started_date'] !="0000-00-00 00:00:00"){
								
								echo date("d, M Y h:i:s a", strtotime($assign_task_arr[$i]['task_started_date'])); 
								...
								if($assign_task_arr[$i]['status']=='3'){
								
								echo date("d, M Y h:i:s a", strtotime($assign_task_arr[$i]['task_close_date']));
								...
								$consumedtime= $assign_task_arr[$i]['consumedtime']; 
						
								 if($consumedtime >=60){ 
								
								   $consumed_hours = floor($consumedtime / 60);
								   
								   $consumed_mints = ($consumedtime  % 60);
								   ...
								   echo  $consumed_hours.":".$consumed_mints;
								
								   
								 }else{
									
									echo  "00:".$consumedtime;
                  ...
                  <?php if($total_allocatedtime !=0 or $total_consumedtime !=0 ){ ?>
                  <table class="table table-striped table-bordered table-hover" id="manage_cms_pages">
                    <thead> 
                    <tr>
                        <th class="">Total Allocated Time: <?php  
						
						
						
						if($total_allocatedtime >=60){ 
								 
						   $hours = floor($total_allocatedtime / 60);
						   $mints = ($total_allocatedtime % 60);
						 
						   $total_allocatedtime=  $hours.":".$mints;
						
						   
						 }else{
							 
						   $total_allocatedtime=  "00:".$total_allocatedtime;
						...
						echo "<span style='color:green;'>".$total_allocatedtime."</span>";?></th>
						...
						if($total_consumedtime >=60){ 
								 
						   $hours = floor($total_consumedtime / 60);
						   $mints = ($total_consumedtime % 60);
						 
						   $total_consumedtime=  $hours.":".$mints;
						
						   
						 }else{
							 
							$total_consumedtime=  "00:".$total_consumedtime;
						...
						echo "<span style='color:green;'>".$total_consumedtime."</span>";?></th>
