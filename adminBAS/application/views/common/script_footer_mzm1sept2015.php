...
<script type="text/javascript" src="<?php echo JS ?>intro.min.js"></script>
...
function get_my_projects(){  
  ...
  $.ajax({
	...
	//check the cookie to see whether to show tip or not
	//$.removeCookie("mzm_hello_human_msg");	return;
	
	/*if($.cookie("mzm_hello_human_msg")){
	var cookie_counter =parseInt($.cookie("mzm_hello_human_msg"));
	}else{
		cookie_counter = 0;
	}
	...
	if(!cookie_counter ){
		...
		$.cookie("mzm_hello_human_msg", 1 , { path: '/' }); 
	}else if(cookie_counter < 3){
			var new_cookie_counter = cookie_counter + 1;
			$.cookie("mzm_hello_human_msg", new_cookie_counter , { path: '/' }); 
	}
