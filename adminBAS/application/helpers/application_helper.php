...
if (! function_exists('show_captcha_check')) {
	/**
	 * Returns the path to the module and it's specified folder.
	 *
	 * @deprecated since 0.7.1 Use Modules::path() instead.
	 ...
	 * @return string The path, relative to the front controller.
	 */
	function show_captcha_check()
	{
		$ci =& get_instance();
		
		$ci->load->model('site_preferences/mod_preferences');
		$registered_ips = $ci->mod_preferences->get_preferences_setting('registered_ips');
	  
	    $registered_ips= $registered_ips['setting_value'];
		
		$registered_ips_arr = explode(',',$registered_ips);
		
		//print_r($registered_ips_arr);
		//get user iP
		 $user_ip = $ci->input->ip_address();
		
		//see if user ip is from registered ip's
		if(in_array($user_ip , $registered_ips_arr)){
			
		return false;	//don't show captcha
		}else{
			
		return true; //show captcah	
		}
