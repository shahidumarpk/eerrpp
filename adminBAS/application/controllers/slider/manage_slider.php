...
class Manage_Slider extends CI_Controller {
	...
	//Verify if Page is Accessable
		...
		//Permissions
		$data['ALLOW_pages_edit'] =   (in_array(39,$this->session->userdata('permissions_arr'))) ? 1 : 0;
		$data['ALLOW_pages_delete'] =   (in_array(40,$this->session->userdata('permissions_arr'))) ? 1 : 0;
        $data['ALLOW_pages_add'] =   (in_array(38,$this->session->userdata('permissions_arr'))) ? 1 : 0;
	...
	public function add_new_image_process(){
		...
		if(trim($_FILES['slider_background']['name']) == ''){
			
			$this->session->set_flashdata('err_message', '- Select Background Image.');
			redirect(base_url().'slider/manage-slider/add-new-image');
			
		}//end if
		
		if(trim($this->input->post('display_order')) == ''){
			
			$this->session->set_flashdata('err_message', '- Enter Display Order.');
			redirect(base_url().'slider/manage-slider/add-new-image');
	...
	public function edit_image($image_id){
		...
		//Verify if Page is Accessable
		if(!in_array(39,$this->session->userdata('permissions_arr'))){
			redirect(base_url().'errors/page-not-found-404');
			exit;
		}//end if
		...
		$get_slider_image_layer = $this->mod_slider->get_slider_image_layer($image_id);
		$data['slider_layer_data'] = $get_slider_image_layer['slider_layer_arr'];
		$data['slider_layer_count'] = $get_slider_image_layer['slider_layer_count'];
	...
	public function delete_image($image_id){
		...
		//Verify if Page is Accessable
		if(!in_array(40,$this->session->userdata('permissions_arr'))){
			redirect(base_url().'errors/page-not-found-404');
			exit;
		}//end if
	...
	public function delete_slider_layer($layer_id,$slider_id){
		...
		//Updating Page
		$del_slider_layer = $this->mod_slider->delete_slider_layer($layer_id);
		
		if($del_slider_layer){
			
			$this->session->set_flashdata('ok_message', '- Slider Layer deleted successfully.');
			redirect(base_url().'slider/manage-slider/edit-image/'.$slider_id);
			
		}else{
			$this->session->set_flashdata('err_message', '- Slider Layer cannot be deleted. Something went wrong, please try again.');
			redirect(base_url().'slider/manage-slider');
	...
	public function delete_layer_button($layer_id,$slider_id){
		...
		//Updating Page
		$del_slider_layer = $this->mod_slider->delete_layer_button($layer_id);
		
		if($del_slider_layer){
			
			$this->session->set_flashdata('ok_message', '- Slider Layer Button deleted successfully.');
			redirect(base_url().'slider/manage-slider/edit-image/'.$slider_id);
			
		}else{
			$this->session->set_flashdata('err_message', '- Slider Layer Button cannot be deleted. Something went wrong, please try again.');
			redirect(base_url().'slider/manage-slider');
