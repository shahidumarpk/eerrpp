...
class Manage_User extends CI_Controller {
	...
	public function user_detail($admin_id,$status=""){
		...
		//Admin Contact data
		$admin_contact_data = $this->mod_admin->get_admin_contact_data($admin_id);
		$data['admin_contact_data'] = $admin_contact_data['admin_contact_arr'];
		$data['admin_contact_count'] = $admin_contact_data['admin_contact_count'];
		
		
		//Admin Bank data
		$admin_bank_data = $this->mod_admin->get_admin_bank_data($admin_id);
		$data['admin_bank_data'] = $admin_bank_data['admin_bank_arr'];
		$data['admin_bank_count'] = $admin_bank_data['admin_bank_count'];
		//Admin Unread Notifications data
		$admin_unread_data = $this->mod_admin->get_admin_unread_data($admin_id);
		
		$data['admin_unread_count'] = $admin_unread_data['admin_unread_count'];
	...
	public function user_ids(){
		...
		//Verify if Page is Accessable
		if(!in_array(211,$this->session->userdata('permissions_arr'))){
			redirect(base_url().'errors/page-not-found-404');
			exit;
		}//end if
		...
		//Bread crum
		$this->breadcrumbcomponent->add('Dashboard', base_url().'dashboard/dashboard');
		$this->breadcrumbcomponent->add('User IDS', base_url().'cms/manage-user');
		$data['breadcrum_data'] = $this->breadcrumbcomponent->output();
		...
		$get_admin_user = $this->mod_admin->get_admin_users();
		$data['admin_user_list'] = $get_admin_user['admin_list_result'];
		$data['admin_user_list_count'] = $get_admin_user['admin_list_result_count'];
		
		$this->load->view('admin/user_ids',$data);
	...
	public function print_letter($user_id,$type){
		...
		$file_name = $this->mod_admin->print_letter($user_id,$type);
		
		if($file_name){
			
			$file_url = "letter_pdf/".$file_name;
	...
	//Ajax Response change-status-active
	public function change_status_active($user_id){
		...
		//State List List
		$change_status_active = $this->mod_admin->change_status_active($user_id);
		
		if($change_status_active){
			
			
			$response= "<button  value='0' class='btn btn-danger btn-gradient ' onClick='change_status_inactive()'>InActive</button>";
	...
	//Ajax Response change-status-inactive
	public function change_status_inactive($user_id){
		...
		//State List List
		$change_status_inactive = $this->mod_admin->change_status_inactive($user_id);
		
		if($change_status_inactive){
			
			
			$response= "<button  value='1' class='btn btn-info btn-gradient' onClick='change_status_active()'>Active</button>";
